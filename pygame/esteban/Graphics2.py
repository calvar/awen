...
def empaquetarInstrucciones(source_path):

    archivoEntrada = open(source_path+"texto_1.txt", 'r')
...
def comprobarCabezales(source_path):
    ...
    #print(instrucciones)
    if(instrucciones[len(instrucciones) - 1].strip() == 'end;'):
        contador += 1  
    else: contador += 0
...
def reescribirInstrucciones(listaInstrucciones, contador, out_path):

    archivoSalida = open(out_path+"texto_2.txt", 'w')
    expresionRegularInstruccion = re.compile('\s*[A-Za-z]*\s*\(*(\d*)\)*\s*\w*')
...
def main(source_path, out_path):
    listaInstrucciones = empaquetarInstrucciones(source_path)
    print(listaInstrucciones)
    contador = comprobarCabezales(source_path)
    print(contador)
    reescribirInstrucciones(listaInstrucciones, contador, out_path)

    
    archivoTexto = open(out_path+"texto_2.txt",'r')
...
source_path = "/home/carlos/Documents/Materias_MACC/semillero/awen/pygame/esteban/"
out_path = "/home/carlos/Documents/Materias_MACC/semillero/awen/pygame/esteban/"


main(source_path, out_path)
